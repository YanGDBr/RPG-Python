...
from ..entrada import ler_tecla, perguntar_sim_nao
...
def explorar(personagem, dungeon_id, *, escrever=None, leitor_tecla=None,
             limpar=None, ler_confirmacao=None, ler_acao_batalha=None):
  """Uma sessão de exploração: anda pelo mapa até topar com algo.

  `ler_acao_batalha` é repassado para `batalhar()` caso a exploração termine
  numa luta — sem isso, testes automatizados travariam esperando teclado real.
  """
  ...
  _resolver_evento(personagem, dungeon_id, andar, escrever, ler_confirmacao, ler_acao_batalha)


def _resolver_evento(personagem, dungeon_id, andar, escrever, ler_confirmacao, ler_acao_batalha=None):
  ...
  if bonus_acessorio:
    ...
    escrever('Seu acessório aumenta a chance de achar a sala do chefe!')

  if random.randint(1, chance_boss) == 1:
    escrever(f'Você encontrou a sala do chefe: {andar.chefe}!')
    if ler_confirmacao(f'Deseja entrar para batalhar contra {andar.chefe}?'):
      _lutar(personagem, dungeon_id, andar.chefe, escrever, ler_acao_batalha)
    return
  ...
  if valor_monstro:
    ...
    escrever('O item que você usou aumenta a chance de encontrar um monstro.')
  if valor_anti:
    peso_monstro = max(1, peso_monstro - valor_anti)
    escrever('O item que você usou reduz a chance de encontrar um monstro.')

  if random.randint(1, peso_monstro + 2) <= peso_monstro:
    nome_monstro = random.choice(andar.monstros_comuns)
    escrever(f'Você encontrou um {nome_monstro}!')
    if ler_confirmacao('Deseja lutar contra ele?'):
      _lutar(personagem, dungeon_id, nome_monstro, escrever, ler_acao_batalha)
    return

  if random.randint(1, 2) == 1:
    moedas = random.randint(5, 15 + andar.numero * 5)
    personagem.moeda_cobre += moedas
    escrever(f'Você encontrou {moedas} moedas de cobre!')
  else:
    escrever('Você explorou a dungeon e não encontrou nada.')


def _lutar(personagem, dungeon_id, nome_monstro, escrever, ler_acao_batalha=None):
  resultado, monstro = batalhar(personagem, MONSTROS[nome_monstro], escrever=escrever,
                                 ler_acao=ler_acao_batalha)
  personagem.local = f'dungeon:{dungeon_id}'
  if resultado == ResultadoBatalha.VITORIA:
    escrever(f'Você derrotou {monstro.nome}!')
