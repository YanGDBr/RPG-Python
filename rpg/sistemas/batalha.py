...
from ..config import (CHANCE_CRITICO_BASE, FOME_CRITICA, MULTIPLICADOR_CRITICO,
...
def calcular_dano(personagem, habilidade, monstro):
  ...
  dano *= multiplicador_elemental(habilidade.elemento, monstro)
...
def personagem_ataca(personagem, habilidade, monstro, escrever):
  if habilidade.tipo == 'ataque_multiplo':
    ...
    escrever(f'Você usa {habilidade.nome} e acerta {acertos}x, '
             f'causando {dano_total} de dano no {monstro.nome}{sufixo}.')
  else:
    dano, critico = calcular_dano(personagem, habilidade, monstro)
    monstro.receber_dano(dano)
    sufixo = ' (CRÍTICO!)' if critico else ''
    escrever(f'Você usa {habilidade.nome}, causando {dano} de dano no {monstro.nome}{sufixo}.')
  ...
  if habilidade.efeito and monstro.vivo:
    ...
    escrever(f'O {monstro.nome} sofreu {habilidade.efeito}!')
...
def monstro_ataca(personagem, monstro, escrever):
  ...
  if random.randint(1, 100) <= esquiva:
    escrever('Você esquiva do ataque do monstro!')
    return
  ...
  escrever(f'{descricao}, causando {dano} de dano em você.')
  ...
  if monstro.base.efeito_aplicado and random.randint(1, 2) == 1:
    ...
    escrever(f'Você sofreu {monstro.base.efeito_aplicado}!')
...
def _tela_batalha(personagem, monstro):
  ...
  return (f'  Batalha contra {monstro.nome}\n'
          f'  Vida do inimigo: {barra(monstro.vida, monstro.base.vida_maxima)}\n\n'
          f'  Sua vida: {barra(personagem.vida, vida_max)}\n'
          f'  Sua mana: {barra(personagem.mana, mana_max)}\n')
...
def batalhar(personagem, monstro_base, escrever=None, ler_acao=None):
  """Roda uma batalha inteira e devolve um ResultadoBatalha.* ."""
  ...
  if efeito_inicial:
    ...
    escrever(f'Seu acessório inflige {nome_efeito} no {monstro.nome} assim que a batalha começa!')

  escrever(f'Você entrou em batalha contra {monstro.nome}!')
  ...
  while True:
    ...
    opcoes = []
    for h in habilidades_ativas:
      cd = personagem.cooldowns.get(h.nome, 0)
      estado = f' [cooldown: {cd}]' if cd > 0 else ''
      opcoes.append(f'{h.nome} ({h.mana} mana){estado}')
    ...
    if escolha < len(habilidades_ativas):
      ...
      if personagem.cooldowns.get(habilidade.nome, 0) > 0:
        escrever('Essa habilidade ainda está em cooldown.')
        continue
      if personagem.mana < habilidade.mana:
        escrever('Mana insuficiente.')
        continue
      ...
      if acao == 'Pular a vez':
        ...
        if random.randint(1, 5) == 1:
          escrever('Você fugiu com sucesso!')
          return ResultadoBatalha.FUGA, monstro
        escrever('Você não conseguiu fugir!')
